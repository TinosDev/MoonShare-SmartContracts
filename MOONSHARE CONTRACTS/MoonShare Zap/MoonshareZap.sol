...
import {IMoonsharePair} from "./IMoonsharePair.sol";
import {IMoonshareRouter02} from "./IMoonshareRouter02.sol";
import {IWETH} from "./IWETH.sol";
import {Babylonian} from "./Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by MoonshareSwap teams.
 */
contract MoonshareZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // MoonshareRouter interface
    IMoonshareRouter02 public moonshareRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from Moonshare)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address MoonshareRouter
    address private moonshareRouterAddress;
     ...
     * @param _moonshareRouter: address of the MoonshareRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _moonshareRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        moonshareRouterAddress = _moonshareRouter;
        moonshareRouter = IMoonshareRouter02(_moonshareRouter);
    ...
    {
        address token0 = IMoonsharePair(_lpToken).token0();
        address token1 = IMoonsharePair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IMoonsharePair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = moonshareRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = moonshareRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IMoonsharePair(_lpToken).token0() || _token0ToZap == IMoonsharePair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IMoonsharePair(_lpToken).token0() || _token1ToZap == IMoonsharePair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IMoonsharePair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IMoonsharePair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IMoonsharePair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = moonshareRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = moonshareRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IMoonsharePair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "MoonshareRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "MoonshareRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        moonshareRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), moonshareRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(moonshareRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(moonshareRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = moonshareRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = moonshareRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = moonshareRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = moonshareRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = moonshareRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
